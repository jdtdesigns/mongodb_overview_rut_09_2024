...
const resolvers = {
  Query: {
    async getSingleUserById(_: any, args: { user_id: string; }) {
    ...
    async getAllNotes() {
      ...
      const notes = await Note.find().populate({
        path: 'user',
        // Populate the sub property or child property of the user object
        populate: {
          path: 'notes'
        }
      });
  ...
  Mutation: {
    ...
    async addNoteForUser(_: any, args: { user_id: string; text: string; }) {
    ...
    async deleteNoteForUser(_: any, args: { user_id: string; note_id: string; }) {
      const note_id = args.note_id;
      const user_id = args.user_id;
    ...
    async addLikeToNote(_: any, args: { user_id: string; note_id: string; }) {
      const note_id = args.note_id;
      const user_id = args.user_id;
